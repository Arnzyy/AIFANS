...
// Anthropic client for vision analysis
...
function isValidFlag(flag: string): flag is ModerationFlag {
  const validFlags: ModerationFlag[] = [
    'face_drift', 'celeb_risk', 'celeb_high_confidence', 'real_person_suspected',
    'faceswap_suspected', 'deepfake_detected', 'minor_appearance_risk',
    'youth_coded_appearance', 'no_face_detected', 'multiple_faces',
    'low_quality_image', 'no_anchors', 'insufficient_anchors',
    'style_inconsistency', 'ai_generated_confirmed', 'real_photo_suspected',
  ];
...
export async function determineStatus(
  ...
  if (
    ...
    (hasAnchors && result.face_consistency_score <= settings.review_min_face_consistency_drop)
  ) {
    return 'pending_review';
  }
  ...
  const canAutoApprove = 
    ...
    (!hasAnchors || result.face_consistency_score >= settings.auto_approve_min_face_consistency);
...
export async function processScanJob(scanId: string): Promise<void> {
  ...
  try {
    ...
    const status = await determineStatus(result, anchors.length >= settings.min_anchors_for_consistency);
...
export async function reviewScan(
  ...
  const newStatus: ModerationStatus = action === 'escalated' ? 'pending_review' : action;
...
async function logModerationAction(entry: Partial<ModerationAuditLog>): Promise<void> {
  try {
...
async function alertAdmins(scanId: string, subject: string, result: VisionScanResult): Promise<void> {
