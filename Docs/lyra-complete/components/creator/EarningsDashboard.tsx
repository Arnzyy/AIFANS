...
import { TrendingUp, DollarSign, ShoppingBag, MessageSquare, AlertTriangle } from 'lucide-react';
...
export function CreatorEarningsDashboard() {
  ...
  const [needsTaxAttention, setNeedsTaxAttention] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [selectedPeriod, setSelectedPeriod] = useState<'today' | 'thisWeek' | 'thisMonth' | 'thisYear' | 'allTime'>('thisMonth');
    ...
    <div className="space-y-6">
      ...
      {needsTaxAttention && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-yellow-500">Tax Profile Required</h3>
            <p className="text-sm text-yellow-400/80 mt-1">{needsTaxAttention}</p>
            <a 
      ...
      <div className="bg-white/5 rounded-xl p-4">
        ...
        <p className="text-xs text-gray-500 mt-3">
          Platform handles AI costs, infrastructure, and support. You focus on content.
        </p>
      </div>
        ...
        {taxProfile && (
          <div className="mt-3 text-sm">
            ...
            {taxProfile.national_insurance_number && (
              <p className="text-gray-500">
                NI: <span className="text-white">••••••{taxProfile.national_insurance_number.slice(-3)}</span>
              </p>
            )}
          </div>
        )}
