...
import { MEMORY_RULES } from './master-prompt';
...
export async function buildChatContext(
  ...
  if (conversation) {
    ...
    if (messages) {
      recentMessages = messages.reverse() as any;
    }
  }
...
function mergeMemory(
  ...
  return {
    ...
    interests: [...new Set([...(existing.interests || []), ...(newFacts.interests || [])])].slice(0, 20),
    preferences: {
      ...existing.preferences,
      ...newFacts.preferences,
    },
    running_jokes: [...new Set([...(existing.running_jokes || []), ...(newFacts.running_jokes || [])])].slice(0, 10),
...
Summary:`;
  ...
  try {
    ...
    const { FORBIDDEN_PATTERNS } = await import('./master-prompt');
...
export async function clearUserMemory(
  ...
  await supabase.rpc('clear_user_memory', { p_user_id: userId });
}
...
export async function exportUserData(
  ...
  const [memory, messages, settings] = await Promise.all([
    ...
    supabase.from('chat_messages').select('*').eq('subscriber_id', userId),
