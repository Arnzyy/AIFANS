...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ['legal_name', 'date_of_birth', 'address_line1', 'city', 'postcode', 'country'];
    ...
    const profile = await updateCreatorTaxProfile(supabase, user.id, {
      ...
      consent_timestamp: body.tax_reporting_consent ? new Date().toISOString() : undefined,
    });
