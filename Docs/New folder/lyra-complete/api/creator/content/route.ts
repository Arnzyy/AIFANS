...
import { saveContentMetadata, getCreatorContent, reanalyzeContent } from '@/lib/ai/content-awareness/content-service';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file_url || !content_type) {
      return NextResponse.json({ error: 'file_url and content_type required' }, { status: 400 });
    }

    if (!['image', 'video'].includes(content_type)) {
      return NextResponse.json({ error: 'content_type must be image or video' }, { status: 400 });
    }
