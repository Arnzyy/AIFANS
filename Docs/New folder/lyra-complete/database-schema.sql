...
CREATE TABLE IF NOT EXISTS ai_personalities (
  ...
  -- Physical & Style traits (detailed optional traits)
  -- Stored as JSONB for flexibility
  physical_traits JSONB DEFAULT '{}',
  -- Structure:
  -- {
  --   "height_range": "5'4 - 5'6",
  --   "body_type": "slim",
  --   "dress_size": "6",
  --   "shoe_size": "5",
  --   "breast_size": "C",
  --   "favourite_outfits": ["bodysuits", "fitted dresses"],
  --   "favourite_lingerie_styles": ["lace", "silk"],
  --   "favourite_lingerie_colours": ["black", "wine"],
  --   "fashion_aesthetic": "minimal chic",
  --   "styling_descriptors": ["elegant", "intentional"],
  --   "hair_colour": "dark brown",
  --   "eye_colour": "green",
  --   "skin_tone": "olive",
  --   "distinguishing_features": ["freckles", "dimples"]
  -- }
  ...
  dynamic VARCHAR(20) DEFAULT 'switch' CHECK (dynamic IN ('submissive', 'switch', 'dominant')),
  ...
  when_complimented VARCHAR(50) DEFAULT 'flirts_back',
  when_heated VARCHAR(50) DEFAULT 'matches_energy',
...
CREATE INDEX idx_ai_personalities_creator ON ai_personalities(creator_id);

ALTER TABLE ai_personalities ENABLE ROW LEVEL SECURITY;

-- Creators manage their own AI
CREATE POLICY "Creators manage own AI personality"
  ON ai_personalities FOR ALL
  USING (auth.uid() = creator_id);

-- Subscribers can view AI personalities (for chat context)
CREATE POLICY "Subscribers can view AI personalities"
  ON ai_personalities FOR SELECT
  USING (true);

CREATE TRIGGER ai_personalities_updated
