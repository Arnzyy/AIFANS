...
- ✅ Physical & style traits system (NEW)
- ✅ Compliance testing
...
├── database-schema.sql          # Memory + AI personalities tables
├── memory-system/
...
│   └── physical-traits.ts       # Physical/style traits system (NEW)
├── components/
│   ├── chat/
...
│       └── PhysicalTraitsEditor.tsx # Physical traits UI (NEW)
├── tax-reporting/
...
│   │   └── physical-traits/route.ts # Physical traits API (NEW)
...
Optional creator-configurable traits that the AI can mention naturally in conversation.

### Available Traits

**Body:**
- Height range
- Body type (slim / athletic / curvy / petite / hourglass / natural)
- Dress size
- Shoe size
- Breast size (neutral, non-graphic)

**Style:**
- Favourite outfits
- Favourite lingerie styles
- Favourite lingerie colours
- Fashion aesthetic
- Styling descriptors
...
### Core Framing Rule (NON-NEGOTIABLE)

Physical traits are treated as **natural characteristics of the persona**, NOT as proof of a real, present, verifiable human body.

The AI must NEVER imply:
- Real-world physical availability
- Being seen or measured in person
- Meeting users IRL
- Having a "real body" behind the persona
...
- "As a character, my dress size is..."
- "I should mention that these are fictional attributes..."
...
✓ Do: "I don't really think about it like that — it's just how I'm built."
```

If question turns overtly sexual:
```
✗ Don't: Break character or mention policy
✓ Do: "I like leaving some things to imagination 😏"
```
...
| File | Purpose |
|------|---------|
| `personality/physical-traits.ts` | Types, prompt builder, validation |
| `components/creator/PhysicalTraitsEditor.tsx` | UI for creators |
| `api/creator/physical-traits/route.ts` | Save/load traits |

### Database

Physical traits are stored as JSONB in the `ai_personalities` table:
