...
CREATE TABLE IF NOT EXISTS sfw_ai_personalities (
  ...
  --   "favourite_outfits": ["sundresses", "jeans"],
  --   "styling_descriptors": ["elegant", "casual"]
  -- }
...
CREATE TABLE IF NOT EXISTS sfw_user_memory (
  ...
  --   "topics_enjoyed": ["music", "movies", "travel"]
  -- }
...
-- =====================
-- MIGRATION: Add default chat modes for existing creators
-- Run this to ensure existing creators keep NSFW enabled
-- =====================

-- INSERT INTO creator_chat_modes (creator_id, nsfw_enabled, sfw_enabled, default_mode)
-- SELECT id, true, false, 'nsfw'
-- FROM auth.users
-- WHERE id IN (SELECT DISTINCT creator_id FROM ai_personalities)
